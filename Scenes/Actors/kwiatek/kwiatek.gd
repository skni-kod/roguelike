...
var gracz = false
var kopia = load("res://Scenes/Actors/kwiatek.tscn")
var KOLOR = 0
var kwiatek = false
...
func atak():
	if gracz:
		...
		global_position = Vector2(floor(gracz.global_position.x + rand_range(-100,100)), gracz.global_position.y + floor(rand_range(-50,50)))

func _on_ziarno_body_entered(body):
	if body.name == "Player":
		queue_free()
