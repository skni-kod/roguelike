...
func _on_AttackTimer_timeout():
	if atakuje and health>0: # gdy przełącznik atakuje jest włączony i kaktus żyje, to wykonuje funkcje
		attack()
# === ======================== === #
...
func get_dmg(dmg, weaponKnockback):
	...
	if health<=0:
		...
		# === UMIERANIE I COINSY === #
		drop_coins()
		emit_signal("died", self) # zostaje wyemitowany sygnał, że kaktus umarł
		queue_free() # instancja kaktusa zostaje usunięta
		# === ================= === #
