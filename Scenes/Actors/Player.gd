...
func take_dmg(dps, enemyKnockback, enemyPos): #otrzymanie obrażeń przez bohatera
	...
	if !skok and !immortal:
		...
		if knockbackResistance != 0:
			...
			knockback /= 0.6
		# ======= ========= ======= #
		health = health - (dps * statusEffect.damageMultiplier) - (dps * ((helmet_resistance * helmet_on)+(breastplate_resistance * breastplate_on)+(pants_resistance * pants_on))) # aktualizacja ilości życia z uwzględnieniem współczynnika damage
